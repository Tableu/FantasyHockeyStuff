...
from features import goalies as goalies_module
...
def parse_args():
    ...
    parser.add_argument("--goalies", action="store_true",
                        help="Build the goalie table instead of the skater one")
...
def candidates_from(lineup: pd.DataFrame, positions=SKATER_POSITIONS) -> pd.DataFrame:
    """The base table's universe: the variant's keys for these positions, one row each."""
    return (lineup.loc[lineup["position"].isin(positions),
                       ["season_id", "game_id", "game_date", "team_id", "player_id", "position"]]
            .drop_duplicates()
            .reset_index(drop=True))


def build_goalie_season(cursor, season, season_id, variant_a, args) -> None:
    """The goalie arm of the build: same shape as the skater one, different grain."""
    candidates = candidates_from(variant_a, GOALIE_POSITIONS)
    log.info("candidate universe: %d goalie rows over %d games",
             len(candidates), candidates["game_id"].nunique())

    goalie_base = goalies_module.build_goalie_base(cursor, [season_id], candidates)
    base_path = paths.FEATURES_DIR / f"goalies_base_{season}.parquet"
    goalie_base.to_parquet(base_path, index=False)
    log.info("goalie base: %d rows x %d columns -> %s",
             len(goalie_base), goalie_base.shape[1], base_path.name)

    lineup = variant_a if args.variant == "A" else lineup_parquet(season, "B", args.copies)
    table = goalies_module.assemble(goalie_base, lineup)
    out_path = paths.FEATURES_DIR / f"goalies_{args.variant}_{season}.parquet"
    table.to_parquet(out_path, index=False)
    log.info("goalie variant %s: %d rows x %d columns -> %s",
             args.variant, len(table), table.shape[1], out_path.name)

    doc = columns_module.render(
        table,
        f"Goalie feature table -- {season}, lineup variant {args.variant}",
        "Generated by `build_feature_table.py --goalies`; every column below is asserted to "
        "exist in the parquet.\nEach feature is computable at the daily lineup lock: see "
        "`features/goalies.py` for the leakage rule.",
    )
    doc_path = paths.DOCS_DIR / f"goalie-feature-table-{args.variant}-{season}.md"
    ...
    if args.verify:
        ...
        verify.run_goalies(cursor, table, goalie_base, candidates, season_id)
...
def main():
    ...
    for season in seasons:
        ...
        if args.goalies:
            build_goalie_season(cursor, season, season_id, variant_a, args)
            continue
